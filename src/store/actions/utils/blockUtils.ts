...
export const blockAlignment = (
  ...
  const middleBlock = selectedBlocks.reduce((acc, curr) => {
    ...
    return (accPosition + acc.width!) / 2 > (currPosition + curr.width!) / 2
      ? curr
      : acc;
  });

  const middlePosition =
    (middleBlock.position[alignment] + middleBlock.width!) / 2;

  const updatedBlocks = get().flowSlice.flow.visual.blocks.map(
    (block: Node<any>) => {
      if (block.selected) {
        const delta = Math.floor(
          middlePosition - (block.position[alignment] + block.width! / 2)
        );
        const newPosition = {
...
export const getSelectedBlock = (flowSlice: FlowSlice): Node<any> => {
  return flowSlice.flow.visual.blocks.find(
    (b: Node<any>) => b.selected === true
  )!;
};
