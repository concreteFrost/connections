...
function EditGroupModal(props: EditGroupModalProps) {
  const { userList, getGroupList, getUserList } = useStore(
  ...
  function removeFromCurrentUsers(userId: string) {
    setCurrentMembers((prevCurrentMembers: any) =>
      prevCurrentMembers.filter((user: User) => user.userId !== userId)
    );
  }
  ...
  function removeFromAvailableUsers(userId: string) {
    setAvailableMembers((prevCurrentMembers: any) =>
      prevCurrentMembers.filter((user: User) => user.userId !== userId)
    );
  }
  ...
  const filteredList = (
    listToFilter: Array<User>,
    listToCheck: Array<User>
  ) => {
    console.log("filtering");
    return listToFilter.filter(
      (availableUser: User) =>
        !listToCheck?.some(
          (currentUser: User) => currentUser.userId === availableUser.userId
        )
    );
  };
  ...
  function filterUsers() {
    ...
    if (props.groupToEdit?.groupMembers)
      setAvailableMembers(
        filteredList(userList, props.groupToEdit?.groupMembers)
      );
  }
        ...
        <div className={s.container}>
          <div className={s.modal_window}>
            <header className={s.modal_header}>
              EDIT {props.groupToEdit?.name.toUpperCase()}
            </header>
            <main className={s.modal_body}>
              <div className={s.form}>
                        ...
                        ? currentMembers.map((user: User) => (
                            <li key={user.userId}>
                              <div className={s.list_item_wrapper}>
                                <p>{user.userName}</p>
                                <button
                                  className={s.delete_from_group_btn}
                                  onClick={() => {
                                    if (props.groupToEdit)
                                      removeFromGroup(
                                        user,
                                        props.groupToEdit?.groupId
                                      );
                                  }}
                                >
                                  X
                ...
                <section className={s.form_btns_wrapper}>
                  <button onClick={() => props.toggleAddGroupModal(false)}>
                    OK
                  </button>
                </section>
              </div>
