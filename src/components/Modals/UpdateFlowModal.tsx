...
function UpdateFlowModal(props: UpdateFlowProps) {
    ...
    async function compareSubfolderName() {
        await getDraftListApi().then((res: any) => {
            ...
            const mergedFolders: any = Object.values(data).reduce((result: any, currentArray) => result.concat(currentArray), []);
            const matchSubfolderName = mergedFolders.find((flow: any) => flow.flowName === flowSlice.flow.flowName)?.subFolder;
    ...
    async function tryToSaveFlow() {
        try {
            ...
            await flowSlice.saveDraftFlow(match, subfolderName);
            ...
            <div className={s.modal_window}>
                ...
                <footer className={s.modal_footer}>
                    <div className={s.input_wrapper}>
                        ...
                        <input type="text" value={subfolderName} onChange={(e: any) => setSubfolderName(e.target.value)} />
                    </div>
