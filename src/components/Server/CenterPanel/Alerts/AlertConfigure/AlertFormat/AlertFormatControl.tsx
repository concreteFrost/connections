...
function AlertFormatControl() {
  const [isAddAlertFormatVisible, setAddAlertFormatVisible] =
    useState<boolean>(false);
  const { getUserList, getGroupList, userList, groupList } = useStore(
  ...
  return (
    <section className={s.wrapper}>
      <header>
        <h3>Alert Format Control</h3>
      </header>
      <div className={s.header_btn_wrapper}>
        <button onClick={() => setAddAlertFormatVisible(true)}>ADD</button>
      </div>

      {isAddAlertFormatVisible ? (
        <AddAlertFormatForm
          setAlertFormatVisible={setAddAlertFormatVisible}
          userList={userList}
          groupList={groupList}
