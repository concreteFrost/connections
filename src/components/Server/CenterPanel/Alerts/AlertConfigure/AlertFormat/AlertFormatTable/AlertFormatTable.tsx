...
function AlertFormatTable(props: AlertFormatTableProps) {
    ...
    useEffect(() => {
        if (props.alertFormats) {
            ...
            const endIndex = Math.min(startIndex + PAGE_SIZE, props.alertFormats.length);
                                ...
                                <tr key={alertFormat.alertFormatId} onClick={() => checkCurrentAlertsIndex(alertFormat.alertFormatId)}>
                                    <td colSpan={2}>
                                        <input
                                            ...
                                            onChange={(e) => updateAlertFormatData(alertFormat.alertFormatId, "name", e.target.value)}
                                        />
                                    </td>
                                    <td colSpan={2} >
                                    ...
                                    <td colSpan={2}>
                                        <select
                                            ...
                                            onChange={(e) => updateAlertFormatData(alertFormat.alertFormatId, "userOrGroupId", e.target.value)}
                                        >
                                    ...
                                    <td colSpan={2} className={s.timestamps}>
                                        <p>Created: {moment(alertFormat.dateCreated).format('lll')}</p>
                                        <p>Last Amended: {moment(alertFormat.lastAmended).format('lll')} </p>
                                    </td>
                                    <td colSpan={1} className={s.checkbox_wrapper}>
                                        <input
                                            type="checkbox"
                                            checked={alertFormat.isActive}
                                            onChange={(e) => updateAlertFormatData(alertFormat.alertFormatId, "isActive", !alertFormat.isActive)}
                                        />
                                    </td>
                                    <td colSpan={1}>
                                        <div className={s.action_btns_wrapper}>
                                            {currentAlertIndex === alertFormat.alertFormatId ? (
                                                <button className={s.save_btn} onClick={() => handleAlertFormatUpdate(alertFormat)} >Save</button>
                                            ) : null}
                                            <button className={s.delete_btn} onClick={() => {
                                                setConfirmationModalActions(() => handleAlertFormatDelete(alertFormat.alertFormatId))
                                                toggleConfirmationModal(true, `Would you like to delete ${alertFormat.name}?`)
                                            }}>DELETE</button>
                                        </div>
                                    </td>
                                </tr>
