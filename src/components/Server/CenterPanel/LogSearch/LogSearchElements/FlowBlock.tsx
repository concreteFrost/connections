...
function FlowBlock(props: FlowBlockProps) {
  ...
  return (
    <section className={s.wrapper}>
      ...
      <select
        value={props.flowId}
        onChange={(e: any) => props.setFlowId(e.target.value)}
      >
        <option value={""}>All Flows</option>
        {props.loadedLiveFlows.length > 0
          ? props.loadedLiveFlows.map((flow: FlowConfig) => (
              <option
                key={props.loadedLiveFlows.indexOf(flow)}
                value={flow.flowId}
              >
                {flow.name}
              </option>
      ...
      <select
        value={props.blockId}
        onChange={(e: any) => props.setBlockId(e.target.value)}
      >
        <option value={""}>All Blocks</option>
        {props.loadedBlocks.length > 0
          ? props.loadedBlocks.map((block: BlockLookup) => (
              <option
                key={props.loadedBlocks.indexOf(block)}
                value={block.blockId}
              >
                {block.name}
              </option>
